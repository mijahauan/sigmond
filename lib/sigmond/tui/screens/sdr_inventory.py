...
def _build_rx888_conf(config_name: str, device_index: int) -> str:
    ...
    return (
        ...
        f"samprate = 12k\n"
        f"mode = usb\n"
        f"ttl = 0               # set to 1 to send multicast out ethernet\n"
        f"\n"
        f"[rx888]\n"
        f"device = \"rx888\"\n"
...
def _write_radiod_conf(entry: SdrEntry, meta: SdrDeviceMeta) -> Optional[str]:
    ...
    content = _build_rx888_conf(config_name, device_index)
